...
def create_workout(workout: WorkoutCreate, user: CurrentUser, db: DbSession):
    return crud_workout.create_workout(workout, user.id, db)


@router.put("/{workout_id}", response_model=WorkoutRead)
def update_workout(workout_id: int, workout_update: WorkoutUpdate, user: CurrentUser, db: DbSession):
